  ...
  return (
    <div className="border rounded-3 p-2">
      <div className="d-flex gap-2 align-items-center mb-2">
        ...
        <button type="button" className="btn btn-light" onClick={selectNone}>None</button>
      </div>
...
export default function SchoolRolesPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const rList = normalizeList(rRes.data);
      const pList = normalizeList(pRes.data);

      setRoles(
        rList.map((r) => ({
          id: r.id,
          name: r.name,
          permissions: normalizeList(r.permissions, undefined),
        }))
      );
            ...
            <div className="card-body">
              <form onSubmit={createRole} className="d-flex flex-column gap-3">
                ...
                <div>
                  ...
                  <PermissionPicker allPerms={allPerms} value={perms} onChange={setPerms} />
                </div>
                    ...
                    {filteredRoles.map((r) => {
                      ...
                      return (
                        <tr key={r.id}>
                          ...
                          <td>
                            <span className="badge bg-primary-light text-primary-600">{permNames.length}</span>
                          </td>
                          <td className="text-truncate" style={{ maxWidth: 360 }}>
                            {permNames.length > 0 ? permNames.join(", ") : "—"}
                          </td>
                          <td>
                            <button className="btn btn-sm btn-success me-1" onClick={() => startEdit(r)}>
                              <Icon icon="lucide:edit" />
                            </button>
                            <button className="btn btn-sm btn-danger" onClick={() => deleteRole(r)}>
                  ...
                  <form onSubmit={saveEdit} className="d-flex flex-column gap-3">
                    <div>
                      ...
                      <input
                        className="form-control"
                        value={editing.name}
                        onChange={(e) => setEditing((old) => ({ ...old, name: e.target.value }))}
                        required
                      />
                    </div>
