...
export default function SchoolPermissionsPage() {
  ...
  const load = async () => {
    ...
    setErr("");
    try {
      const { data } = await API.get("school/permissions");
  ...
  const handleSubmit = async ({ id, name }) => {
    ...
    try {
      if (id) {
        await API.put(`school/permissions/${id}`, { name });
        showFlash("Permission updated");
      } else {
        await API.post("school/permissions", { name });
  ...
  const deletePerm = async (p) => {
    ...
    try {
      await API.delete(`school/permissions/${p.id}`);
